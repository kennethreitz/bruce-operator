...
class Operator(object):
    ...
    def lookup_resource(self, api_version, kind):
        ...
        resources = self.generic.call_api(f"/{api_prefix}/{api_version}", "GET")[0][
            "resources"
        ]
    ...
    def delete_thirdparty(self, thirdparty):
        ...
        for api_version, kind in kinds:
            url = (
                self.build_url(api_version, kind, namespace)
                + f"?labelSelector=app={name},thirdparty={thirdparty_kind}"
            )
            print(f"DELETE {url}")
            try:
        ...
        for event in watch.stream(
            ...
            if event["type"] == "ADDED" and event["object"]["kind"] == kind:
                self.apply_templates(
                    path=Path() / kind.lower() / "manifests", values=event["object"]
                )
